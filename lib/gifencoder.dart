...
Uint8List makeGif(int width, int height, List<int> rgba) {
  final b = new GifBuffer(width, height)..add(rgba);
  return b.build(1);
}
...
class GifBuffer {
  ...
  Uint8List build(int framesPerSecond) {
    ...
    final result = new Uint8List(len);
    int offset = 0;
    for (var chunk in bytes) {
...
class _ColorTableBuilder {
  ...
  Uint8List indexImage(int width, int height, List<int> rgba) {
    final pixels = new Uint8List(width * height);
  ...
  _ColorTable build() {
    for (int bits = 1; bits <= 8; bits++) {
      ...
      if (colors * 3 >= table.length) {
        final copy = new Uint8List(colors * 3)
          ..setRange(0, table.length, table);
...
List<int> _header(int width, int height, int colorBits) {
  ...
  final bytes = <int>[]..addAll(_headerBlock);
...
void _addShort(List<int> dest, int n) {
  if (n < 0 || n > 0xFFFF) throw new Exception("out of range for short: $n");
